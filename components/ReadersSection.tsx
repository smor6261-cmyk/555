...
const ReadersSection: React.FC<ReadersSectionProps> = ({ isActive }) => {
    ...
    const renderInfoContent = () => {
        ...
        switch(activeInfo) {
            case 'intro':
                content = <>
                    <h3 className="text-2xl font-bold text-[#2c5f2d] mb-3">التعريف بالقارئ: {selectedReader.name}</h3>
                    <p className="text-gray-700">{selectedReader.intro}</p>
                </>;
                break;
            case 'teachers':
                 content = <>
                    <h3 className="text-2xl font-bold text-[#2c5f2d] mb-3">أهم شيوخ {selectedReader.name}</h3>
                    <p className="text-gray-700">{selectedReader.teachers}</p>
                </>;
                break;
            case 'narrators':
                content = <>
                    <h3 className="text-2xl font-bold text-[#2c5f2d] mb-3">رواة {selectedReader.name}</h3>
    ...
    const getSubButtonClass = (infoType: ReaderInfoType) => {
    ...
    return (
        <section className="bg-white rounded-xl p-6 shadow-md mb-8 transition-opacity duration-500 ease-in-out animate-fadeIn">
            <h2 className="text-3xl font-bold text-center text-[#2c5f2d]">القراء العشرة</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mt-6">
                {readersData.map(reader => (
                    <Card key={reader.id} title={reader.name} onClick={() => handleReaderSelect(reader)} />
                ))}
            </div>
            ...
            {selectedReader && (
                <div ref={detailsRef} className="mt-10 transition-all duration-500 ease-in-out">
                    <div className="flex justify-center gap-4 my-5 flex-wrap">
                        <button className={`text-white px-6 py-2 rounded-full font-semibold transition-colors ${getSubButtonClass('intro')}`} onClick={() => setActiveInfo('intro')}>التعريف بالقارئ</button>
                        <button className={`text-white px-6 py-2 rounded-full font-semibold transition-colors ${getSubButtonClass('teachers')}`} onClick={() => setActiveInfo('teachers')}>أهم شيوخه</button>
                        <button className={`text-white px-6 py-2 rounded-full font-semibold transition-colors ${getSubButtonClass('narrators')}`} onClick={() => setActiveInfo('narrators')}>رواته</button>
                    </div>
                    {renderInfoContent()}
                </div>
            )}
